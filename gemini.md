...
* **What we build:** User registration, password hashing (bcrypt), session management with JWT access tokens, long-lived refresh token rotation, and API key generation (e.g., prefix `sk_live_...`).
...
* **What we build:** A reverse proxy built using Go's `net/http/httputil`, a dynamic configuration engine reading routes from a YAML file, and a token-bucket rate limiter backed by Redis.
...
* **What we build:** The Gateway fires a background event to Redis Streams on request completion. The Usage Service consumes this via a consumer group, batches the database writes to PostgreSQL to avoid lock contention, and emits a quota breach event if limits are exceeded.
...
* **What we build:** Plan management (Free vs. Pro) and idempotent invoice processing using idempotency keys to ensure safety across retries.
...
```
       ...
       (1) Fire-and-Forget: Publish Message
             ┌───────────────────┐
             │   REDIS STREAMS   │  <- [Topic: request.completed]
             └─────────┬─────────┘
                       │
         ┌─────────────┴─────────────┐
...
1. **Publish Event:** The Gateway fires a structured log message into Redis Streams:
   ```go
   redis.XAdd("request.completed", map[string]interface{}{
       "user_id":   99,
...
2. **Asynchronous Ingestion (Usage Service):**
   * The Usage Service consumes events from the `request.completed` stream.
